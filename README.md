...
> **基于 [yangskin/UEBlueprintMCP](https://github.com/yangskin/UEBlueprintMCP) 修改扩展。**  
> **基于 yangskin/UEBlueprintMCP 基于 lilklon/UEBlueprintMCP(https://github.com/lilklon/UEBlueprintMCP) 修改。**  
> 原项目采用 **MIT 协议**，本插件在其基础上进行了大量重构与功能增强，同样遵循 MIT 协议发布。

---

一个面向 AI 辅助开发的 MCP 插件，用于 Unreal Engine 5.7+ 编辑器自动化。  
对外暴露 **7 个固定 MCP 工具**，由后端 **动作注册表（Action Registry）** 驱动，支持 259 个动作，提供持久 TCP 连接、多客户端支持与自动保存。

同时包含一个可选的**独立日志 MCP 服务器**（`ue-editor-mcp-logs`），暴露 `unreal.logs.get`、`unreal.asset_thumbnail.get`、`unreal.asset_diff.get` 和 `unreal.asset_history.get` 四个工具。
...
-   **259 个动作** — 蓝图、图节点、材质、UMG 控件、MVVM、增强输入、组件事件绑定、PIE 启停/状态查询、日志断言、Outliner 管理、资产缩略图提取、Level Viewport 截图、静态网格体尺寸查询与摆放、编辑器 Python 执行、Niagara、批量资产重命名及重定向修复、与版本控制仓库的历史/差异对比 — 均可通过动作注册表动态发现
-   **Niagara MCP 自动化** — 99 个 `niagara.*` 动作覆盖 System、Emitter、Module、Parameter、Renderer、Runtime、Scratch Pad 与 Niagara Graph 编辑
-   **编辑器 Python 执行** — `editor.execute_python` 可在当前 Unreal Editor 会话中运行 Python，并通过 `result` 返回结构化结果
-   **资产与蓝图清理** — 支持删除关卡 Actor、Content Browser 资产、图节点、变量、函数、UMG Widget 蓝图、Niagara System、材质表达式等
...
VS Code / MCP 客户端（GitHub Copilot 等）
  ...
  C++ 插件（FMCPServer → 每连接一个 FMCPClientHandler）
        ...
        │  游戏线程分发
        ▼
  FEditorAction 子类（177 个处理器）→ 校验 → 执行 → 自动保存
        │
        ▼
  Unreal Editor
```
...
-   `unreal.asset_history.get`（需要 UE 编辑器 + 源码控制连接，用于列出资产提交历史并辅助选择 diff revision）
...
| 域             | 数量 | 说明                                                                                 | 示例 ID                                                                                                                                                                                                                                                      |
| -------------- | ---- | ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `blueprint.*`  | 11   | 蓝图创建、编译、属性、父类、接口、组件、快照                                         | `blueprint.create`、`blueprint.compile`、`blueprint.add_component`、`blueprint.describe_full`                                                                                                                                                                |
...
| `editor.*`     | 36   | 关卡 Actor、资产、视口、日志、PIE、Outliner、生命周期、Python 执行、源码控制历史/差异 | `editor.execute_python`、`editor.delete_asset`、`editor.get_static_mesh_infos`、`editor.spawn_static_mesh_actor`、`editor.capture_viewport`、`editor.list_assets`、`editor.get_logs`、`editor.is_ready`、`editor.request_shutdown`                                |
| `layout.*`     | 4    | 节点自动布局和注释                                                                   | `layout.auto_selected`、`layout.auto_subtree`、`layout.auto_blueprint`、`layout.layout_and_comment`                                                                                                                                                          |
| `node.*`       | 24   | 蓝图图节点创建、通用节点搜索、异步节点、自演进建议                                   | `node.add_event`、`node.add_function_call`、`node.add_branch`、`node.add_generic`、`node.search_catalog`                                                                                                                                                     |
...
| `graph.*`      | 20   | 图连线、检视、注释、补丁、折叠重构、节点导入导出                                     | `graph.connect_nodes`、`graph.describe`、`graph.export_nodes`、`graph.import_nodes`、`graph.collapse_selection_to_function`、`graph.auto_comment`、`graph.apply_patch`                                                                                       |
| `macro.*`      | 1    | 宏管理                                                                               | `macro.rename`                                                                                                                                                                                                                                               |
| `material.*`   | 17   | 材质创建、编辑、编译诊断、图检视与布局、关卡应用、编辑器刷新、选中节点读取           | `material.create`、`material.add_expression`、`material.compile`、`material.get_summary`、`material.auto_layout`、`material.auto_comment`、`material.remove_expression`、`material.apply_to_component`、`material.apply_to_actor`、`material.refresh_editor` |
| `widget.*`     | 22   | UMG 控件蓝图（24 种类型）、控件删除/重命名、MVVM                                     | `widget.create`、`widget.delete`、`widget.add_component`、`widget.delete_component`、`widget.mvvm_add_viewmodel`、`widget.mvvm_remove_viewmodel`                                                                                                             |
| `input.*`      | 4    | 增强输入系统                                                                         | `input.create_action`、`input.create_mapping_context`                                                                                                                                                                                                        |
| `niagara.*`    | 99   | Niagara System、Emitter、Module、Parameter、Renderer、Runtime、Scratch Pad、Graph    | `niagara.create_system`、`niagara.add_emitter`、`niagara.set_module_input`、`niagara.add_renderer`、`niagara.spawn_effect`、`niagara.create_scratch_pad_module`、`niagara.connect_pins`                                                                       |

### 静态网格体摆放闭环

```json
{
    "action_id": "editor.get_static_mesh_infos",
    "params": {
        "static_meshes": ["/Engine/BasicShapes/Cube.Cube", "/Engine/BasicShapes/Sphere.Sphere"]
    }
}
```
...
```json
{
    "action_id": "editor.capture_viewport",
    "params": {}
}
```
...
```json
{
    "action_id": "editor.spawn_static_mesh_actor",
    "params": {
...
```json
{
    "action_id": "editor.set_static_mesh_actor_material",
    "params": {
...
`editor.execute_python` 可在当前 Unreal Editor 进程中执行 Python 代码，适合调用 Unreal Python API、触发项目内生成流程或查询编辑器状态。执行环境会预导入 `unreal` 模块；若脚本中给变量 `result` 赋值，该值会作为结构化结果返回。

```json
{
    "action_id": "editor.execute_python",
    "params": {
        "code": "assets = unreal.EditorUtilityLibrary.get_selected_assets()\nresult = [asset.get_path_name() for asset in assets]"
    }
}
```
...
> `editor.execute_python` 属于高权限动作，可修改资源、执行控制台命令或关闭编辑器。调用前应明确目标，并优先使用专门动作完成常见编辑器操作。

### 资产删除与清理

注册表中可直接发现的删除/清理动作包括：

-   `editor.delete_actor`：删除当前关卡中的 Actor
-   `graph.delete_node`：删除蓝图图节点
-   `variable.delete` / `function.delete`：删除蓝图成员变量或自定义函数图
-   `widget.delete` / `widget.delete_component`：删除 UMG Widget 蓝图资产或 Widget Tree 中的控件
-   `material.remove_expression`：删除材质表达式节点并断开相关连接
-   `niagara.delete_system` / `niagara.remove_emitter` / `niagara.remove_module` / `niagara.remove_renderer` / `niagara.delete_scratch_pad_module` / `niagara.delete_graph_node`：删除 Niagara 资源或内部元素

-   `editor.delete_asset`：删除 Content Browser 资产；执行前会关闭已打开的资源编辑器，参数为 `asset_path`

### Niagara MCP

Niagara 能力由 99 个 `niagara.*` 动作组成，覆盖常见特效资产生产闭环：

-   System：创建、列出、删除、编译、读取错误、设置系统属性、播放范围
-   Emitter：添加、移除、复制、重命名、排序、属性设置、模板列表
-   Module：查询 Stack、添加/删除/排序模块、启停模块、读取/设置输入、动态输入、曲线、快速迭代参数
-   Parameter：用户参数增删改查、类型列表、参数链接、Data Interface schema
-   Renderer：添加/删除 Renderer、读取/设置 Renderer 属性与绑定
-   Runtime：在关卡中生成 Niagara Actor、控制播放、给 Actor 添加 NiagaraComponent、读取运行实例统计
-   Scratch Pad / Graph：创建 Scratch Pad、Apply/Apply & Save、Custom HLSL、脚本参数、图节点增删、Pin 管理、连线/断线、类型与函数搜索

完整参数与返回字段见 [`docs/Niagara.md`](docs/Niagara.md)。
...
```powershell
& "E:\CPP_UnrealEngine\UE_5.7\UnrealEngine\Engine\Build\BatchFiles\Build.bat" LingqiResetEditor Win64 Development -Project="E:\My Project\LingqiReset\LingqiReset.uproject" -WaitMutex
```
...
-   `assetPath`（必填）：完整资产路径（如 `/Game/Blueprints/BP_Foo`）
-   `maxCount`（可选）：最多返回多少条实际修改过该资源的版本记录
-   返回源码控制历史记录，通常用于先查 revision，再把指定版本传给 `unreal.asset_diff.get`
-   需要 UE 编辑器运行且源码控制（SVN/Perforce/Git）已连接
...
-   动作注册表含 259 个 ActionDef 条目，支持关键字搜索和模式自省
...
| `Python/ue_editor_mcp/registry/actions.py`  | 259 个带模式/标签/示例的 ActionDef 条目             |
| `Python/ue_editor_mcp/server_unreal_logs.py` | 独立日志/缩略图/资产历史/资产差异 MCP 服务器        |
| `docs/Niagara.md`                           | Niagara MCP 动作参数与返回字段说明                  |
...
| `Source/Private/MCPBridge.cpp`              | 动作处理器注册表（177 条命令）                      |
...
7 工具统一 MCP 服务器，动作注册表（259 个动作）、关键字搜索、批量执行、嵌入式文档和结构化日志——**全部完成**。
...
本项目基于 [yangskin/UEBlueprintMCP](https://github.com/yangskin/UEBlueprintMCP)（MIT 许可证）修改扩展。  
